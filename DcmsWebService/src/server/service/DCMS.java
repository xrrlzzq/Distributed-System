...
/**
 * This is the implement of the remote interface
 */
@WebService
public class DCMS implements DcmsRemoteInterface{
	...
	static String myServer;//mtl,lvl,ddo
	private LogTool log;
	...
	public void init(String mtlIP,String lvlIP,String ddoIP,int udpServerPort,int udpClientPort,String serverLocation,LogTool log) throws IOException{
		this.mtlIP = mtlIP; 
		this.lvlIP = lvlIP; 
		this.ddoIP = ddoIP; 
		this.udpServerPort = udpServerPort;
		//this.udpClientPort = udpClientPort;
		DCMS.myServer = serverLocation.toLowerCase();
		this.log = log;
	...
	public String getRecordCounts(String managerID) {
		...
		String result="";
		synchronized(records){ //use database hashmap as lock to make thread safe(synchronized) when update HashMap data
		switch(myServer){ //get the count of records from local server and send the udp request to other two server to get its records count respective
		       case "mtl":
	...
	public boolean checkRecordID(String recordID){
		...
		if(recordID.length()!=8) // if the length of record id not equal to 8, then it is invalid(eg MTR00001 is valid)
			...
			if(reVaildSet.indexOf(leftCheck)==-1)// check if the first 3 prefix of record id is MTR,LTR,DTR,MSR,LSR,DSR
				return false;
			try{
				int num=Integer.parseInt(rightCheck);//check if the rest of record id is number
			}catch(Exception e){
				result=false;
			}
		}
	...
	public String transferRecord(String managerID, String recordID, String remoteCenterServerName) {
		...
		synchronized(records){ //use database hashmap as lock to make thread safe(synchronized) when update HashMap data  
			...
			if(newRecord.equals(""))
				return log.logMsg("record "+recordID+" not exist", false, "server."+myServer,"transferRecord");
			...
			case "mtl":result=log.logMsg(transferUDPRecord(mtlIP ,newRecord), true, "server."+myServer,"transferRecord");break;
			
			case "lvl":result=log.logMsg(transferUDPRecord(lvlIP ,newRecord), true, "server."+myServer,"transferRecord");break;
			
			case "ddo":result= log.logMsg(transferUDPRecord(ddoIP ,newRecord), true, "server."+myServer,"transferRecord");break;
			}
	 ...
	 */
	@WebMethod(exclude=true)
	public  String transferUDPRecord(String ip,String recordinfo){
    	String result="";
    	byte[] receiveBuf=new byte[1024];
    	DatagramPacket dpReceive=new DatagramPacket(receiveBuf,receiveBuf.length);
    	try{
            ...
            //socket.setSoTimeout(10);
            String info="t"+recordinfo; // if the request start with "t", then it is the request for transferring
            byte[] data=info.getBytes();
            InetAddress addr = getInetAddress(ip);
            DatagramPacket dpSend=new DatagramPacket(data,data.length,addr,udpServerPort);
            socket.send(dpSend);
	 ...
	 */
	public static String dealTransferedRecord(String requestInfo){ //insert into hash map database at other server, so does not need synchronized
		String result="";
		String[] newRecord=requestInfo.substring(1).split("\\|");
		HashMap<String,String> record=new HashMap<String,String>();
    	if(newRecord.length==8){ // do the same action as createTR 
    		 char initial=newRecord[2].charAt(0);
			   ...
			   result=String.format("transfe teacher record :(%s,%s,%s,%s,%s,%s,%s) into server[%s] by manager[%s] successfully",newRecord[0],
		    ...
		    result=String.format("transfe student record :(%s,%s,%s,%s,%s,%s) into server[%s] by manager[%s] successfully",newRecord[0],
